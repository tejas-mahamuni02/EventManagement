...
<body>
  ...
  <div class="container-fluid bg-image " id="home">
    ...
    <div class=" m-auto   fs-3 fw-bold text-center border border-white border-5 div1 bg-color mb-5 text-dark"
      style="transform: rotate(3deg);">Scroll To Book Package
    </div>
  ...
  <div class="container-fluid text-center w-75 m-auto p-5 " style="color: #555555;" id="allbookings">
    <h1 class="mt-5">Fill The Moments To Make Joy </h1>
    <c:choose>
      <c:when test="${empty allbookings}">
        <h1 class="mt-5">There is No Bookings Yet </h1>

      </c:when>
      <c:otherwise>
        <h6  id="filterlable">You can Use Only One Filter From Given Filters</h6>
        <div class="input-group">
          <input type="text" id="searchInput" class="form-control" placeholder="By event..."
            onkeyup="filterEvents()">
          <input type="date" id="searchInput1" class="form-control" placeholder="By Date..."
            onchange="filterEventsByDate()">
        </div>
        <div class="input-group mt-2">
          <input type="text" id="searchInput2" class="form-control" placeholder="By Customer Name..."
            onkeyup="filterEventsByCustomer()">
          <input type="text" id="searchInput3" class="form-control" placeholder="By Location..."
            onkeyup="filterEventsByLocation()">
        </div>
          <button class="btn btn-secondary mt-2" onclick="resetFilters()">Reset Filters</button>


        <div class="cards my-5">
          <div class="row row-cols-1 row-cols-md-1 g-5" id="bookingContainer">

            <c:forEach var="booking" items="${allbookings}">
              <c:set var="present" scope="page" value="false" />
              <div class="col booking-item ">
                <div class="card p-3 border border-5 bg-light ">
                  <h5 >Booking id : ${booking.bookingid }</h5>
                  <div class="card-body row row-cols-1 row-cols-md-2 g-2  border-bottom border-3 p-3">
                    <div class="col">
                      Type Of Event:-<h4 class="card-title event">${booking.event.event}</h4>
                    </div>
                    <div>
                      Name Of Customer:- <h4 class="customer" >${booking.user.name} (${booking.user.id})</h4>

                    </div>
                  </div>


                  <h5 class="mt-2">Rates of Package  </h5>
                  <h5>Package id:- ${booking.event.eventid }  </h5>
                  <div class="card-body row row-cols-1 row-cols-md-4 g-2 border-bottom border-3 p-3">
                    <div class="col">
                      Price of meal for one person: <br>
                      <h6> ${booking.event.platerate } &#8377;</h6>
                    </div>
                    <div class="col">
                      Entertainment Cost:<br>
                      <h6>${booking.event.entertainment }&#8377;</h6>
                    </div>
                    <div class="col">
                      Chair Rate per 100 Person:<br>
                      <h6>${booking.event.chairrate }&#8377;</h6>
                    </div>
                    <div class="col">
                      Decoration Rate:<br>
                      <h6>${booking.event.decoration }&#8377;</h6>
                    </div>
                  </div>
                  <div class="card-body row row-cols-1 row-cols-md-3 g-2 ">
                    <div class="col fs-5 ">
                      Event Location: <br>
                      <h5 class="location"> ${booking.location} </h5>
                    </div>
                    <div class="col fs-5 ">
                      Event Date: <br>
                      <h5 class="book-date"> ${booking.date} </h5>
                    </div>
                    <div class="col fs-5 ">
                      Payment For This Event: <br>
                      <h5> ${booking.total} &#8377;</h5>
              ...
              <c:set var="total" scope="page" value="${total+booking.total}" />
        ...
        <div class="row row-cols-1 row-cols-md-2 g-2 ">
          <div class="col fs-5 fw-bold">
            Total Event Booked till Date:
            <h5> ${i} </h5>
          </div>
          <div class="col fs-5 fw-bold">
            You Paid For All Events till date: <br>
            <h5> ${total} &#8377;</h5>
          </div>
        </div>
    ...
    <div class="row  g-4 mt-5">
      <a href="homepage" class="btn bg-color fs-5 px-5 border border-black fw-bold col-md-4  m-auto ">Go To Home Page</a>
...
<script>
  function filterEvents() {
    ...
    const items = container.getElementsByClassName('booking-item');

    for (let i = 0; i < items.length; i++) {
      const title = items[i].getElementsByClassName('event')[0].innerText.toLowerCase();
    ...
    document.getElementById('searchInput3').disabled = true;
     document.getElementById('filterlable').innerHTML = "Click Reset filter Button To Use Another"; 
    document.getElementById('filterlable').style.color = "red"; 

  }
  function filterEventsByDate() {
	    const inputDate = document.getElementById('searchInput1').value; // Get the input value
	    const container = document.getElementById('bookingContainer');
	    const items = container.getElementsByClassName('booking-item');

	    for (let i = 0; i < items.length; i++) {
	      const eventDate = items[i].getElementsByClassName('book-date')[0].innerText; // Get the event date text

	      if (eventDate === inputDate) { // Compare the date exactly
	        items[i].style.display = "";
	      } else {
	        items[i].style.display = "none";
	      }
  ...
  function filterEventsByCustomer() {
    ...
    for (let i = 0; i < items.length; i++) {
      const title = items[i].getElementsByClassName('customer')[0].innerText.toLowerCase();
      if (title.indexOf(filter) > -1) {
        items[i].style.display = "";
      } else {
        items[i].style.display = "none";
  ...
  function filterEventsByLocation() {
    ...
    for (let i = 0; i < items.length; i++) {
      const title = items[i].getElementsByClassName('location')[0].innerText.toLowerCase();
      if (title.indexOf(filter) > -1) {
        items[i].style.display = "";
      } else {
        items[i].style.display = "none";
  ...
  function resetFilters() {
    const input = document.getElementById('searchInput');
    const inputDate = document.getElementById('searchInput1');
    const inputCustomer= document.getElementById('searchInput2');
    const inputLocation= document.getElementById('searchInput3');
    const container = document.getElementById('bookingContainer');
    const items = container.getElementsByClassName('booking-item');
    ...
    document.getElementById('filterlable').style.color = "black"; 
    document.getElementById('filterlable').innerHTML = "You can Use Only One Filter From Given Filters";
  }
</script>
